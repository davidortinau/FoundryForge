// FoundryForge in-app theme control. The design tokens (tokens.css) already define light
...
(function () {
    ...
    function pushBackground() {
        try {
            ...
            if (window.DotNet && bg) {
                window.DotNet.invokeMethodAsync('FoundryForge.App', 'SetWindowBackgroundColor', bg);
            }
    ...
    function syncNativeWindow(mode, attempt) {
        ...
        try {
            if (window.DotNet) {
                window.DotNet.invokeMethodAsync('FoundryForge.App', 'SetWindowThemeMode', mode);
                pushBackground();
    ...
    window.foundryForgeTheme = { apply, current, syncWindowBackground };
