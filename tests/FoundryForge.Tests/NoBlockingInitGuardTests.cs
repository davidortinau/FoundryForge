...
public class NoBlockingInitGuardTests
{
    ...
    private static string FindRepoRoot([CallerFilePath] string? thisFile = null)
    {
        ...
        while (dir is not null && !File.Exists(Path.Combine(dir, "FoundryForge.sln")))
        {
            dir = Path.GetDirectoryName(dir);
        }

        return dir ?? throw new DirectoryNotFoundException($"FoundryForge.sln not found above {thisFile}");
    }
}
