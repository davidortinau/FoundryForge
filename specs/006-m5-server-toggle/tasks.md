...
- **Layering (Constitution V / DEC-004)**: `FoundryForge.App` consumes **only** `ILocalServerService` + the Core seams (`ServerState`/`ServerStatus`, `ServerStateMachine`, `ServerEndpoints`, `ServerLimitations`, `RequestActivityProjection`) + the M1 lifecycle/gate — **never** `Microsoft.AI.Foundry.Local`/FL types (FR-024). All FL types stay inside `FoundryForge.Foundry`; `LocalServerService` is the **only** new FL-bound class.
...
- [ ] T001 Confirm a green baseline: build the solution and run `dotnet test tests/FoundryForge.Tests/FoundryForge.Tests.csproj` from repo root; record the existing-suite pass set (incl. `NoBlockingInitGuardTests`, `ModelStateGateTests`, `RamFitHeuristicTests`, `SettingsDocumentTests`, `CatalogGroupingTests`, `DeleteConsentGateTests`, `PostV1StubTests`, …) so regressions are visible. Confirm the existing honest stub `src/FoundryForge.Core/PostV1/StubLocalServerService.cs` (`IsSupported => false`; Start/Stop throw `"wired in M5"`) and the disabled `nav-server` `<button>` in `src/FoundryForge.App/Components/Layout/Sidebar.razor` are present as the M5 starting point. **No** new `PackageReference` is added in M5.
...
- `src/FoundryForge.App/Components/Pages/Server.razor` + its view-state are edited by T016 (scaffold), T019 (US1 endpoint/empty surface) — **serialize these edits** (and any later region added inline).
- `src/FoundryForge.App/Components/Layout/Sidebar.razor` is edited by T015 only.
- `src/FoundryForge.App/MauiProgram.cs` is edited by T014 only.
- `src/FoundryForge.Foundry/LocalServerService.cs` is created by T013 only.
...
Task: "ServerStateMachineTests in tests/FoundryForge.Tests/ServerStateMachineTests.cs"
Task: "ServerEndpointsTests in tests/FoundryForge.Tests/ServerEndpointsTests.cs"
Task: "ServerLimitationsTests in tests/FoundryForge.Tests/ServerLimitationsTests.cs"
Task: "RequestActivityProjectionTests in tests/FoundryForge.Tests/RequestActivityProjectionTests.cs"
Task: "ServerGateCoordinationTests in tests/FoundryForge.Tests/ServerGateCoordinationTests.cs"

# Then implement the pure seams in parallel (different new files):
Task: "ServerStateMachine.cs in src/FoundryForge.Core/Server/"
Task: "ServerEndpoints.cs (+ ServerRoute) in src/FoundryForge.Core/Server/"
Task: "ServerLimitations.cs in src/FoundryForge.Core/Server/"
Task: "RequestActivityProjection.cs in src/FoundryForge.Core/Server/"
```
