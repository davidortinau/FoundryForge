...
- **Decision**: `ISettingsService` (Core) over `AppSettings` (record: `ModelCacheDirectory`, `DefaultModel`, `Theme`). The **pure logic** — documented defaults, JSON (de)serialization, merge-missing-with-defaults, and a `RequireConsent` guard that prevents destructive overwrite/clear without an explicit consent flag — lives in `FoundryForge.Core.Settings.SettingsDocument` (FL-free, fully unit-tested). The **platform binding** — persisting the JSON via MAUI Essentials `Preferences` (key) plus a human-readable JSON file in app data — lives in `FoundryForge.Foundry.PreferencesSettingsService`. On missing/empty/corrupt read, apply documented defaults **without** destroying prior data (non-destructive recovery: keep a `.bak` of an unparseable file).
...
- **Decision**: Four projects: FL-free `FoundryForge.Core` (interfaces + DTOs + pure seams + gate primitive), FL-bound `FoundryForge.Foundry` (FL impls + adapter + stubs), `FoundryForge.App` (`net10.0-macos` Razor head, imports the bundle target + entitlements), and `FoundryForge.Tests` referencing **Core only**. The bundle target is imported **only** by the head.
