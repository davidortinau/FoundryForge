...
### DEC-019 — Design system: FoundryStudio Design Guide + token foundation + Sherpa shell (autonomous)
**Date:** 2026-06-26 · **Status:** Active
**Context:** Before chat (M4) + server (M5) land, established a durable, enforceable design system so every surface is consistent, honest, and accessible. Synthesized from a competitive/sibling review (LM Studio, GitHub Copilot App, MAUI Sherpa) + a GPT-5.5 creative-direction pass that David approved the direction for (dual light/dark, hybrid Fluent+signature identity, Sherpa-style sidebar, doc + token foundation + catalog restyle, explore a forge motif).
**Identity decided — "Foundry Copper" + the forge-as-honest-state metaphor:**
- Single brand accent **Foundry Copper** (`#B94A18` light / `#FF9E4A` dark) on a Fluent-2 structure; copper NEVER means health/safety/capability — semantic colors own meaning.
- Two themes at parity: **Workshop Daylight** (light) + **Night Forge** (dark, "forge at night").
- **No mascot** (LM Studio owns that) — a stamped copper "foundry mark" instead.
- Forge metaphor is **state language only**, used when literally true: "Cast to local cache" (download), "Temper in memory" (load), "Forge lit at <url>" (server). Reinforces the Constitution's honesty/transparency value.
- Sherpa-style grouped sidebar shell (Models active; Chat/Server clearly-disabled "Coming soon" placeholders seeded for M4/M5; Settings).
**Delivered:** `docs/DESIGN.md` (the enforceable guide, linked from AGENTS.md); `wwwroot/tokens.css` (103 `--fs-*` tokens — color/type/space/radius/elevation/motion; light `:root`/`[data-theme=light]` + dark `prefers-color-scheme` AND `[data-theme=dark]` for a future toggle); refactored `app.css` to consume tokens; `Components/Layout/` Sherpa shell (MainLayout/Sidebar/PageHeader); restyled catalog (Home/ModelCard/Settings) to tokens+components. All M3 DOM hooks + consent/honesty invariants preserved.
**Review:** independent code-review (Hicks implemented, separate reviewer — reviewer independence) — consent/data-preservation, honesty, KI-005, DOM hooks, token discipline, shell honesty, a11y all CLEAN. 2 findings FIXED + re-verified: [Med] `.shell` rule removed broke the AppReadyBoundary startup/init-failure gates (a surface screenshots don't cover) — restored token-based `.shell`; [Low/Med honesty] format badge hardcoded "ONNX" when the SDK reported no type — now omits the badge when blank (matches the card's unknown-handling).
**Verified:** real Apple Silicon, net11.0-macos — `dotnet build FoundryStudio.sln` 0 errors; **51/51 xUnit pass** (KI-005 guard green); catalog renders in **both** Workshop Daylight and Night Forge via the `[data-theme]` toggle (`--fs-accent` resolves `#B94A18`→`#FF9E4A`, canvas `#F7F4EF`→`#0E0D0B`); sidebar shell with copper-active nav + disabled Chat/Server placeholders; forge voice live; capability chips semantic green (not copper); **consent gate intact post-restyle** — dialog names model+GB+"cannot be undone", danger button + Cancel copper-ring default focus, cancel preserves David's cached qwen2.5-0.5b (count stays 1, NO live delete). Evidence: docs/evidence/design-catalog-light.png, design-catalog-dark.png, design-consent-dialog-dark.png.
**Open follow-ups:** chat composer/metrics + server panel are designed in the guide but built in M4/M5; in-app theme toggle UI not yet surfaced (tokens support it); onboarding/first-run + foundry-mark asset are guide concepts, not yet implemented.

### DEC-020 — M4 Streaming chat (v1 core) COMPLETE (autonomous; net11.0-macos)
**Date:** 2026-06-26 · **Status:** Active
**Context:** The v1 lighthouse payoff — multi-turn streaming chat with a locally-loaded model, fully in-process (no loopback socket; the M5 server is for external tools only). Built specify→plan→tasks→implement→e2e→review on the M1 in-process `IChatClient` adapter. Followed docs/DESIGN.md (composer, metrics row, forge voice) + the spec's honesty/consent emphasis.
**Delivered (US1–US7):** streaming multi-turn chat with markdown (Markdig) + code-copy; loaded-model gate + load-on-demand; stop/cancel (persists partial + UserCancelled); honest token metrics (TTFT, tok/sec, total, stop reason); per-chat system prompt + EXACTLY the four FL-supported params (temperature/max-tokens/top-p/frequency-penalty); honest context-window estimate; chat history persisted to app data (one JSON per conversation) with consent-gated clear/delete. Core dylib-free seams (28 new unit tests): TranscriptAssembler, InferenceParameters, TokenStatsAccumulator, ContextWindowEstimator, ChatMarkdown, ChatHistoryDocument, FileChatHistoryStore, IChatHistoryStore + models (ChatSession/ChatMessageRecord/GenerationMetrics/ContextWindowEstimate). Foundry wiring: ChatService builds `adapter.AsBuilder().UseFunctionInvocation().UseOpenTelemetry().Build()`; FoundryChatClient applies ONLY the four params to `OpenAIChatClient.Settings` (never the TopK/RandomSeed/PresencePenalty that also exist — honesty) and extracts real usage/finish-reason (FoundryMessageMapper).
**Decisions made in David's absence:**
- D1: **Deferred US8 live tool-calling UI + US9 structured-output toggle** (KI-011). The MEAI pipeline (`UseFunctionInvocation`) + `ToolInvocationWiringTests` prove the middleware is real and ready, but the full MEAI↔Betalgo tool bridge + hardware verification need a reliably tool-capable model (qwen2.5-0.5b is too small; downloading a larger one unprompted costs time/bandwidth). Shipping unverified tool UI would violate Constitution III — so NO dead tool UI ships; the capability is honestly absent.
- D2: FL streaming emits NO usage frame, so `metric-total` honestly shows "unknown" (KI-010) — TTFT/tok-sec are still real; finish reason IS provided. This is the honest outcome research R2 predicted, not a bug.
- D3: Placed `FileChatHistoryStore` in Core (App injects the path) so the consent gate + persistence are unit-testable dylib-free.
**Review:** independent code-review (coordinator authored Core+Foundry, Hicks authored UI; separate reviewer — reviewer independence). Consent/data-preservation, honesty (4 params, honest unknowns), KI-005, layering, deferred-feature absence, DOM hooks, a11y all CLEAN. **1 HIGH fixed + re-verified:** assistant markdown allowed `javascript:`/`vbscript:`/`file:` URI injection (XSS) — `.DisableHtml()` blocks raw HTML but not URL schemes, and the BlazorWebView interop context made it exploitable. Fixed in `ChatMarkdown` by scheme-filtering link/image URLs (allow http/https/mailto, data: for images, relative) on the parsed document before rendering; +6 regression tests.
**Verified:** real Apple Silicon, net11.0-macos — `dotnet build FoundryStudio.sln` 0 errors; **85/85 xUnit pass** (KI-005 guard green). DevFlow DOM e2e on qwen2.5-0.5b: nav-chat enabled → gate → load-on-demand → model loaded; **streaming is INCREMENTAL** (345→1120→7603 chars, `data-streaming` true→false — real stream, not a post-hoc paste); metrics TTFT 374ms + 70.9 tok/sec (real) + total "unknown" (FL no usage, honest) + stop "max-tokens" (real finish reason, proving the 2048 max-tokens param applied); context "Estimate: ~1,647 of 32,768 tokens (5%)" (labeled estimate, real denominator); multi-turn (2 user/2 assistant); **exactly 4 param controls, zero unsupported**; history persisted + **SURVIVED RESTART** (FR-023); delete dialog names the conversation + "cannot be undone" + Cancel default-focus + cancel preserves David's conversation (no live delete). Both themes resolve tokens on the chat surface (light #B94A18/#F7F4EF, dark #FF9E4A/#0E0D0B). **Pixel screenshots not captured — KI-001: the WebView is blank unless the window is genuinely frontmost, which isn't achievable driving the app headlessly; DOM inspection is the by-design evidence path.** Data preservation honored: David's cached model never deleted; only test-created conversations touched.
**Open follow-ups:** KI-011 (live tool-calling UI + FL tool bridge + tool-capable-model verification); KI-010 (FL streaming usage — informational); optional CSP meta as XSS defense-in-depth (needs its own BlazorWebView verification); US9 structured-output/regenerate (post-v1).
